...
- **Encoder facts — `weights/MANIFEST.md`.** For each of CBraMod, BENDR, LaBraM: embedding dim, project epoch input length (`epoch_input_s`, with `native_window_s` retained only as a backward-compatible alias if needed), sampling rate, expected montage, finest supported granularity, loss type, pretraining datasets — plus provenance fields (source URL, version, file format, local path, load quirk) for later use. Facts must be filled in now; weight files and paths are used only at Gate 3. Load facts from this file; do not fetch anything over the network.
  - Distinguish `epoch_input_s`, `patch_s`, and `pretraining_crop_s`; if encoder facts appear inconsistent, overloaded, or underspecified, do not guess silently. Pause and ask the user, or document any assumptions in `docs/HANDOFF.md` if the current gate can continue safely. When resolving an inconsistency, update the source doc that caused it (`MANIFEST.md`, `DESIGN.md`, or `BUILD_SPEC.md`) so future gates inherit the clarified rule.
...
**Gate 0.5 — encoder facts into the registry.** Read `weights/MANIFEST.md` and populate the adapter registry for the trio: embedding dim, project epoch input length (`epoch_input_s` / `native_window_s` alias), patch length if applicable, sampling rate, expected montage, finest granularity, loss, pretraining datasets. This allows the dummy adapter to emit correct shapes and the pretraining-overlap flag has data.

...
2. `scripts/analyze.py` reads all experiments and builds a structured summary for each one: config deltas from the previous run, key metrics, a trajectory signal (whether the model was still improving at the final epoch), and the researcher's notes.
...
reports/            Generated reports (gitignore-able)
scripts/
